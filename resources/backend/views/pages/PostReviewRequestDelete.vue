      ...
      <div class="col-12">
        ...
        <div class="panel panel-bordered">
          <div class="panel-heading">
            <h3 class="panel-title">Danh sách bài viết yêu cầu xóa</h3>
          </div>
...
export default {
  ...
  methods: {
    async getPostReviews({ page, filter, sort }) {
      try {
        const response = await PostReviewService.getPostReviewRequestDelete({
          page: page,
          text: filter,
    ...
    deletePost(id) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          PostReviewService.delete(id).then(response => {
            if (response) {
              Swal.fire({
                type: "success",
