...
class SearchController extends Controller
{
    ...
    public function postReviewSearchFilTer(Request $request)
    {
        $data =  $request->only(['rating', 'money', 'city', 'district']);
        $post_reviews = PostReview::where('post_reviews.is_approve', 1)->whereHas('user', function ($query) {
            $query->where('status', 1);
        })->whereHas('shop', function ($query) use ($data) {
            if ($data['city'] !== null) {
                $query->with('city')->where('city_id', $data['city']);
            }
            if ($data['district'] !== null) {
                $query->with('district')->where('district_id', $data['district']);
            }
        })->with(['shop' => function ($query) {
            $query->select('id', 'name',);
        }]);
        if ($data['money'] === "0") {
            $post_reviews = $post_reviews->where('stars', $data['rating'])->paginate();
        } else {
            $post_reviews = $post_reviews->where('stars', $data['rating'])->where('money', $data['money'])->paginate();
        }
